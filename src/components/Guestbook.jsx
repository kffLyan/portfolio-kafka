...
import { Send, Check, Copy, ArrowUpRight, MessageSquareQuote, Mail } from 'lucide-react';
import confetti from 'canvas-confetti';
import { PERSONAL_INFO, INITIAL_GUESTBOOK } from '../data/portfolioData';
 ...
 * Direct Instagram transmission, email copy, and optimistic local guestbook.
 */
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        localStorage.setItem(STORAGE_KEY, JSON.stringify(INITIAL_GUESTBOOK));
      }
    } catch (e) {
      setEntries(INITIAL_GUESTBOOK);
    }
  ...
  return (
    <section id="guestbook" className="py-24 sm:py-32 px-6 sm:px-10 md:px-16 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        {/* Right Column: Local Interactive Guestbook Form & Stream */}
        <div className="lg:col-span-7 flex flex-col">
          <div className="p-6 sm:p-8 rounded-3xl bg-bone/70 border border-linen/90 mb-10 shadow-[0_4px_24px_rgba(26,36,33,0.02)]">
            <div className="flex items-center gap-2 mb-6 text-xs font-mono uppercase tracking-wider text-ink font-semibold">
              <MessageSquareQuote className="w-4 h-4 text-terracotta" />
              <span>Buku Tamu / Testimoni (Local Persistence)</span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Tulis pesan, catatan evaluasi, atau feedback teknis..."
              ...
              <div className="flex justify-between items-center pt-2">
                <span className="text-[11px] font-mono text-stone">
                  Optimistic update &bull; Tersimpan di localStorage
                </span>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-ink text-sand text-xs font-mono uppercase tracking-wider hover:bg-terracotta transition-colors duration-200 cursor-pointer disabled:opacity-50"
                >
              ...
              <AnimatePresence initial={false}>
                {entries.map((entry) => (
                  <motion.div
                    ...
                    className="p-5 rounded-2xl bg-bone/40 border border-linen/80"
                  >
